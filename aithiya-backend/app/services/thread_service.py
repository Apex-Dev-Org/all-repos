...
from app.db.repositories import threads_repo


async def create_thread(client, *, user_id: UUID, title: str) -> dict:
    return await threads_repo.insert_thread(client, user_id=user_id, title=title)


async def list_owned(client, *, user_id: UUID, limit: int, offset: int) -> list[dict]:
    return await threads_repo.list_threads(client, user_id=user_id, limit=limit, offset=offset)


async def require_owned(client, *, thread_id: UUID, user_id: UUID) -> dict:
...
async def update_title(client, *, thread_id: UUID, user_id: UUID, title: str) -> dict:
    await require_owned(client, thread_id=thread_id, user_id=user_id)

    row = await threads_repo.update_thread(
        client, thread_id=thread_id, user_id=user_id, title=title
    )
...
async def archive_thread(client, *, thread_id: UUID, user_id: UUID) -> dict:
